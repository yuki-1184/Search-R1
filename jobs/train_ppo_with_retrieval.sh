...
RAY_TMPDIR=${RAY_SHORT_TMPDIR:-/tmp/ray_${PJM_JOBID:-train}}
...
echo "Launching PPO training on GPUs: ${CUDA_VISIBLE_DEVICES}"
singularity exec --nv \
    --bind "$PROJ:/workspace" \
    --bind "$DATA:$DATA" \
    ...
    "$SIF" \
    bash -c "cd /workspace && export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} RETRIEVAL_PORT=${PORT} && bash train_ppo.sh"
